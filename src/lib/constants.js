// ── Athletic standards (median h/w/speed by tier & position) ──────────────────
export const ATH_STANDARDS = {
  "Power 4": {
    QB:   { h50:75, w50:210, s50:4.60 }, RB:   { h50:72, w50:215, s50:4.45 },
    FB:   { h50:72, w50:240, s50:4.65 }, TE:   { h50:76, w50:250, s50:4.60 },
    WR:   { h50:74, w50:190, s50:4.45 }, OL:   { h50:77, w50:300, s50:5.00 },
    C:    { h50:77, w50:300, s50:5.00 }, G:    { h50:77, w50:300, s50:5.00 },
    T:    { h50:78, w50:290, s50:5.00 }, DL:   { h50:75, w50:300, s50:4.85 },
    DE:   { h50:76, w50:260, s50:4.65 }, DT:   { h50:75, w50:300, s50:4.85 },
    EDGE: { h50:76, w50:245, s50:4.55 }, LB:   { h50:73, w50:235, s50:4.55 },
    CB:   { h50:72, w50:190, s50:4.45 }, S:    { h50:73, w50:205, s50:4.50 },
  },
  "G6": {
    QB:   { h50:74, w50:200, s50:4.70 }, RB:   { h50:71, w50:205, s50:4.55 },
    FB:   { h50:71, w50:230, s50:4.75 }, TE:   { h50:74, w50:240, s50:4.75 },
    WR:   { h50:73, w50:185, s50:4.55 }, OL:   { h50:75, w50:290, s50:5.20 },
    C:    { h50:75, w50:290, s50:5.20 }, G:    { h50:75, w50:290, s50:5.20 },
    T:    { h50:76, w50:280, s50:5.20 }, DL:   { h50:74, w50:285, s50:5.00 },
    DE:   { h50:74, w50:250, s50:4.80 }, DT:   { h50:74, w50:285, s50:5.00 },
    EDGE: { h50:74, w50:235, s50:4.70 }, LB:   { h50:72, w50:225, s50:4.65 },
    CB:   { h50:71, w50:180, s50:4.55 }, S:    { h50:71, w50:195, s50:4.60 },
  },
  "1-FCS": {
    QB:   { h50:73, w50:195, s50:4.75 }, RB:   { h50:70, w50:195, s50:4.60 },
    FB:   { h50:70, w50:220, s50:4.80 }, TE:   { h50:73, w50:230, s50:4.85 },
    WR:   { h50:72, w50:180, s50:4.60 }, OL:   { h50:74, w50:280, s50:5.30 },
    C:    { h50:74, w50:280, s50:5.30 }, G:    { h50:74, w50:280, s50:5.30 },
    T:    { h50:75, w50:270, s50:5.30 }, DL:   { h50:72, w50:275, s50:5.10 },
    DE:   { h50:73, w50:240, s50:4.90 }, DT:   { h50:72, w50:275, s50:5.10 },
    EDGE: { h50:73, w50:225, s50:4.80 }, LB:   { h50:71, w50:215, s50:4.70 },
    CB:   { h50:70, w50:175, s50:4.60 }, S:    { h50:71, w50:190, s50:4.65 },
  },
  "2-Div II": {
    QB:   { h50:72, w50:190, s50:4.80 }, RB:   { h50:69, w50:190, s50:4.65 },
    FB:   { h50:69, w50:210, s50:4.85 }, TE:   { h50:72, w50:225, s50:4.95 },
    WR:   { h50:71, w50:175, s50:4.65 }, OL:   { h50:73, w50:270, s50:5.40 },
    C:    { h50:73, w50:270, s50:5.40 }, G:    { h50:73, w50:270, s50:5.40 },
    T:    { h50:74, w50:260, s50:5.40 }, DL:   { h50:71, w50:260, s50:5.20 },
    DE:   { h50:72, w50:235, s50:5.00 }, DT:   { h50:71, w50:260, s50:5.20 },
    EDGE: { h50:72, w50:220, s50:4.90 }, LB:   { h50:70, w50:205, s50:4.75 },
    CB:   { h50:69, w50:170, s50:4.65 }, S:    { h50:70, w50:185, s50:4.70 },
  },
  "3-Div III": {
    QB:   { h50:71, w50:180, s50:4.90 }, RB:   { h50:68, w50:180, s50:4.75 },
    FB:   { h50:68, w50:200, s50:4.95 }, TE:   { h50:71, w50:215, s50:5.05 },
    WR:   { h50:70, w50:170, s50:4.75 }, OL:   { h50:72, w50:260, s50:5.50 },
    C:    { h50:72, w50:260, s50:5.50 }, G:    { h50:72, w50:260, s50:5.50 },
    T:    { h50:73, w50:250, s50:5.50 }, DL:   { h50:70, w50:250, s50:5.30 },
    DE:   { h50:71, w50:225, s50:5.10 }, DT:   { h50:70, w50:250, s50:5.30 },
    EDGE: { h50:71, w50:210, s50:5.00 }, LB:   { h50:69, w50:195, s50:4.85 },
    CB:   { h50:68, w50:165, s50:4.70 }, S:    { h50:69, w50:175, s50:4.75 },
  },
};

// Recruit reach radius in miles by athletic tier
export const RECRUIT_BUDGETS = {
  "Power 4":   2500,
  "G6":        1500,
  "1-FCS":     1000,
  "2-Div II":  600,
  "3-Div III": 450,
};

export const TIER_ORDER = ["Power 4", "G6", "1-FCS", "2-Div II", "3-Div III"];

// SAT nationally-representative percentiles (matching GrittyOS DB SAT_percent tab)
// Values are percentile/100 (e.g. 1250 → 86th percentile → 0.86)
export const SAT_PERCENTILES = {
  1600:0.999,1590:0.998,1580:0.997,1570:0.996,1560:0.995,1550:0.994,
  1540:0.993,1530:0.992,1520:0.991,1510:0.990,1500:0.990,1490:0.990,
  1480:0.990,1470:0.990,1460:0.990,1450:0.990,1440:0.980,1430:0.980,
  1420:0.980,1410:0.970,1400:0.970,1390:0.970,1380:0.960,1370:0.960,
  1360:0.950,1350:0.940,1340:0.940,1330:0.930,1320:0.930,1310:0.920,
  1300:0.910,1290:0.900,1280:0.890,1270:0.880,1260:0.870,1250:0.860,
  1240:0.850,1230:0.840,1220:0.830,1210:0.820,1200:0.810,1190:0.800,
  1180:0.780,1170:0.770,1160:0.760,1150:0.740,1140:0.730,1130:0.710,
  1120:0.700,1110:0.690,1100:0.670,1090:0.650,1080:0.630,1070:0.610,
  1060:0.600,1050:0.580,1040:0.560,1030:0.540,1020:0.520,1010:0.500,
  1000:0.480,990:0.470,980:0.450,970:0.430,960:0.410,950:0.390,
  940:0.370,930:0.360,920:0.340,910:0.320,900:0.310,890:0.290,
  880:0.280,870:0.270,860:0.250,850:0.240,840:0.230,830:0.220,
  820:0.210,810:0.200,800:0.190,
};

export const POSITIONS = ["QB","RB","FB","TE","WR","OL","C","G","T","DL","DE","DT","EDGE","LB","CB","S"];

// Colors keyed to school.Type (athletic tier from GrittyOS DB)
export const DIV_COLORS = {
  "Power 4":   "#f5a623",
  "G6":        "#4fc3f7",
  "FBS Ind":   "#ce93d8",
  "1-FCS":     "#81c784",
  "2-Div II":  "#ef9a9a",
  "3-Div III": "#b0bec5",
};

// US state geographic centroids for recruit-reach distance approximation
export const STATE_CENTROIDS = {
  AL:[32.779,-86.829], AK:[64.446,-153.494], AZ:[34.274,-111.660], AR:[34.894,-92.443],
  CA:[37.184,-119.470], CO:[38.997,-105.548], CT:[41.622,-72.727], DE:[38.990,-75.505],
  FL:[28.631,-82.450], GA:[32.642,-83.443], HI:[20.293,-156.374], ID:[44.351,-114.613],
  IL:[40.042,-89.197], IN:[39.894,-86.282], IA:[42.075,-93.496], KS:[38.494,-98.380],
  KY:[37.535,-85.302], LA:[31.069,-91.997], ME:[45.370,-69.243], MD:[39.055,-76.791],
  MA:[42.260,-71.808], MI:[44.347,-85.410], MN:[46.281,-94.305], MS:[32.736,-89.668],
  MO:[38.357,-92.458], MT:[47.053,-109.633], NE:[41.538,-99.795], NV:[39.329,-116.631],
  NH:[43.681,-71.581], NJ:[40.191,-74.673], NM:[34.407,-106.113], NY:[42.954,-75.527],
  NC:[35.556,-79.388], ND:[47.450,-100.466], OH:[40.286,-82.794], OK:[35.538,-96.925],
  OR:[43.934,-120.558], PA:[40.878,-77.800], RI:[41.676,-71.556], SC:[33.917,-80.896],
  SD:[44.444,-100.226], TN:[35.858,-86.351], TX:[31.476,-99.331], UT:[39.321,-111.094],
  VT:[44.069,-72.666], VA:[37.522,-78.854], WA:[47.383,-120.447], WV:[38.641,-80.623],
  WI:[44.624,-89.994], WY:[42.996,-107.551], DC:[38.904,-77.017],
};

// EFC Determine table — direct port of 'EFC Determine' sheet
// Each row: { agi, sai:[dep1,dep2,dep3,dep4], pub:[...], priv:[...], elite:[...] }
// pub = Avail_FA_All, priv = Avail_FA_Priv4_Elite, elite = Avail_FA_Elite
// Deps capped at 4; lookup uses largest row where row.agi <= athlete AGI (MATCH type=1)
export const EFC_TABLE = [
  { agi:  30000, sai:[     0,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  32500, sai:[     0,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  35000, sai:[     0,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  37500, sai:[     0,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  40000, sai:[     0,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  42500, sai:[  1680,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  45000, sai:[  2122,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  47500, sai:[  2564,     0,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  50000, sai:[  3066,  1504,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  52500, sai:[  3448,  2387,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  55000, sai:[  3890,  2387,     0,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  57500, sai:[  4332,  2829,  1410,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  60000, sai:[  4774,  3271,  1852,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  62500, sai:[  5216,  3713,  2294,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  65000, sai:[  5703,  4155,  2736,     0], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  67500, sai:[  6205,  4597,  3178,  1598], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  70000, sai:[  6752,  5039,  3620,  2040], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  72500, sai:[  7334,  5502,  4062,  2482], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  75000, sai:[  7953,  6004,  4504,  2924], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  80000, sai:[  9334,  7101,  5396,  3808], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  85000, sai:[ 10883,  8314,  6364,  4660], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  90000, sai:[ 12517,  9558,  7374,  5448], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi:  95000, sai:[ 14170, 10965,  8513,  6328], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 100000, sai:[ 15824, 12614,  9792,  7332], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 105000, sai:[ 17477, 14267, 11235,  8463], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 110000, sai:[ 19130, 15920, 12889,  9734], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 115000, sai:[ 20783, 17573, 14542, 11167], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 120000, sai:[ 22437, 19227, 16195, 12820], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 125000, sai:[ 24090, 20880, 17848, 14474], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 130000, sai:[ 25743, 22533, 19501, 16127], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 135000, sai:[ 27396, 24186, 21155, 17780], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 140000, sai:[ 29050, 25839, 22808, 19433], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 145000, sai:[ 30703, 27493, 24461, 21087], pub:[1,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 150000, sai:[ 32356, 29146, 26114, 22740], pub:[0,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 155000, sai:[ 34009, 30799, 27768, 24393], pub:[0,1,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 160000, sai:[ 35662, 32452, 29421, 26046], pub:[0,0,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 165000, sai:[ 37316, 34106, 31074, 27699], pub:[0,0,1,1], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 170000, sai:[ 38969, 35759, 32727, 29353], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 175000, sai:[ 40622, 37412, 34380, 31006], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 180000, sai:[ 42258, 39048, 36016, 32642], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 185000, sai:[ 43864, 40654, 37623, 34248], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 190000, sai:[ 45470, 42260, 39229, 35854], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 195000, sai:[ 47077, 43867, 40835, 37460], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 200000, sai:[ 48683, 45473, 42441, 39067], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 205000, sai:[ 50289, 47079, 44047, 40673], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 210000, sai:[ 51895, 48685, 45654, 42279], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 215000, sai:[ 53502, 50291, 47260, 43885], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 220000, sai:[ 55108, 51898, 48866, 45492], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 225000, sai:[ 56714, 53504, 50572, 47098], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 230000, sai:[ 58320, 55110, 52079, 48708], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 235000, sai:[ 59926, 56716, 53685, 50310], pub:[0,0,0,0], priv:[1,1,1,1], elite:[1,1,1,1] },
  { agi: 240000, sai:[ 61533, 58323, 55291, 51196], pub:[0,0,0,0], priv:[0,1,1,1], elite:[1,1,1,1] },
  { agi: 245000, sai:[ 63139, 59929, 56879, 53523], pub:[0,0,0,0], priv:[0,1,1,1], elite:[1,1,1,1] },
  { agi: 250000, sai:[ 64745, 61535, 58503, 55129], pub:[0,0,0,0], priv:[0,0,1,1], elite:[1,1,1,1] },
  { agi: 275000, sai:[ 72776, 69566, 66535, 63160], pub:[0,0,0,0], priv:[0,0,0,0], elite:[0,1,1,1] },
  { agi: 300000, sai:[ 80982, 77772, 74741, 71366], pub:[0,0,0,0], priv:[0,0,0,0], elite:[0,0,0,0] },
];

export const TIER_LABELS = {
  "Power 4":   { short:"P4",  color:"#f5a623" },
  "G6":        { short:"G6",  color:"#4fc3f7" },
  "FBS Ind":   { short:"Ind", color:"#ce93d8" },
  "1-FCS":     { short:"FCS", color:"#81c784" },
  "2-Div II":  { short:"D2",  color:"#ef9a9a" },
  "3-Div III": { short:"D3",  color:"#b0bec5" },
};
